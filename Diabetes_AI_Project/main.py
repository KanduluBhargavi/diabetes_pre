...
def predict(data: PredictIn, user: User = Depends(get_user)):
    ...
    if data.heart_disease == 1:          penalty += 0.03
    if data.smoking_status == "current": penalty += 0.04
    if data.smoking_status == "former":  penalty += 0.02
    if data.hypertension == 1 and data.heart_disease == 1:           penalty += 0.02
    if data.hypertension == 1 and data.smoking_status == "current":  penalty += 0.02

    adj = min(0.99, prob + penalty)
    return {
        "risk": adj,
